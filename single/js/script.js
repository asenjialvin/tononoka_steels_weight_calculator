document.addEventListener('DOMContentLoaded', () => {
  ...
  // Adjusted sheet width for shearing (keep as is)
  ...
  slitCalcBtn.addEventListener('click', () => {
      ...
      // Use w + 10 as per the requirement (not adjustedWidth)
      const denominator = w + 10;
      ...
      coils.forEach((c, idx) => {
          ...
          slits.forEach(s => {
              // Updated formula: (s.width * c.weight) / (w + 10)
              const weightPerSlit = (s.width * c.weight) / denominator;
              const totalWeightForSize = Math.round(weightPerSlit) * s.count;
          ...
          // Updated formula: scrapWidth = (w - totalSlitWidth) + 10
          ...
          perCoilRows.push({
              ...
              coilWeight: c.weight,
              slitDetails,
              coilSlitsWeight, // Store the total slit weight for this coil
              scrapWidth,
              scrapWeight,
      ...
      // per-coil table - with merged "All Slit Weight" cell (matching output.jpg format)
      const tb = document.querySelector('#slit-summary-table tbody');
      tb.innerHTML = '';
      
      perCoilRows.forEach(r => {
          const rowCount = r.slitDetails.length || 1; // If no slits, still 1 row
          
          // Add rows for each slit size in this coil
          r.slitDetails.forEach((slit, i) => {
              const tr = document.createElement('tr');
              if (i === 0) {
                  // First row: show coil details and merged All Slit Weight
                  tr.innerHTML = `
                      <td rowspan="${rowCount}">${r.number}</td>
                      <td rowspan="${rowCount}">${fmt0(r.coilWeight)}</td>
                      <td>${slit.size}</td>
                      <td>${fmt0(slit.weightPerSlit)}</td>
                      <td>${fmt0(slit.totalWeight)}</td>
                      <td rowspan="${rowCount}">${fmt0(r.coilSlitsWeight)}</td>
                      <td rowspan="${rowCount}">${fmt0(r.scrapWidth)}</td>
                      <td rowspan="${rowCount}">${fmt0(r.scrapWeight)}</td>
                      <td rowspan="${rowCount}">${fmt0(r.grandTotal)}</td>
                  `;
              } else {
                  // Subsequent rows: only slit details
                  tr.innerHTML = `
                      <td>${slit.size}</td>
                      <td>${fmt0(slit.weightPerSlit)}</td>
                      <td>${fmt0(slit.totalWeight)}</td>
      ...
      // totals - match the output.jpg format
          ...
          card.innerHTML = `
              ...
              <div class="coil-mini-grid">
                  ...
                  <div class="form-group checkbox-group" style="grid-column: span 2;">
                      <input type="checkbox" class="form-check-input shear-coil-completed" id="${cardId}-completed">
                      <label class="form-check-label" for="${cardId}-completed">Coil Completed (Fully Sheared)</label>
                  </div>
